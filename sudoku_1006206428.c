...
int check_square(int sudoku[9][9], int row, int col, int value){
  //Top-left coordinate for initializing "regions"
  int startRow = row - row % 3, startCol = col - col % 3;

  //Check Validity of value's inner 3x3 box
  ...
  for (int row=0; row<9; row++){
    for (int col=0; col<9;col++){
      if (sudoku[row][col] != 0){
        ...
        /*
        if (!check_col(sudoku, row, col, sudoku[row][col])){
          printf("check_col() failed: %d %d %d\n", row, col, sudoku[row][col]);
          return 0;
        }
        else if (!check_row(sudoku,row,col,sudoku[row][col])){
          printf("check_row() failed: %d %d %d\n", row, col, sudoku[row][col]);
          return 0;
        }
        else if (!check_square(sudoku, row, col, sudoku[row][col])){
          printf("Square 3x3 failed %d %d %d\n", row, col, sudoku[row][col]);
          return 0;
        }
      }
      */
  ...
  for (int row = 0; row<9; row++){
    for (int col = 0; col<9; col++){
      if (!check_col(sudoku, row, col, sudoku[row][col]) || 
          !check_row(sudoku, row, col, sudoku[row][col]) || 
          !check_square(sudoku, row, col, sudoku[row][col]) || 
          sudoku[row][col]==0) //If there exists at least one 0.
...
int solve(int sudoku[9][9]){
  ...
  if (!board_valid(sudoku)){
    //printf("Original Board is invalid -> USER ERROR\n");
    return 0;
  }

  //Find 0 in the board (Position to solve)
  int row, col; // y,x
  for (row = 0; row<9; row++){
    for (col = 0; col<9; col++){
      if (sudoku[row][col]==0){
  ...
  for(int n = 1; n<=9; n++){
    //print_sudoku(sudoku); //Uncommment to visualzie entire animation
